...
void MainWindow::atualizarTabela(Incidencia x, int linhas)
{
    ...
    if(x.getRisco() == 0){
         ui->tableWidget->setItem(linhas, 6, new QTableWidgetItem("Baixo"));
    }else if(x.getRisco() == 1){
        ui->tableWidget->setItem(linhas, 6, new QTableWidgetItem("Médio"));
    }else if (x.getRisco() == 2) {
        ui->tableWidget->setItem(linhas, 6, new QTableWidgetItem("Alto"));
    }
